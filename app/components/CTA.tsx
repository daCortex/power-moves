...
export default function CTA() {
  return (
    <section id="about" className="relative scroll-mt-28 overflow-hidden border-b border-line-strong/15 bg-ink">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-grid opacity-[0.07]" />
      </div>
      <div className="hazard h-2.5 w-full" />

      <div className="relative mx-auto max-w-[88rem] px-6 py-20 md:py-28">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr] lg:items-end">
          <Reveal>
            ...
            <motion.div whileHover={{ scale: 1.005 }} className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#contact" className="group inline-flex items-center justify-center gap-3 bg-red px-8 py-4 tech-label text-white transition-colors hover:bg-amber hover:text-ink">
                Start Your Project
                <span className="transition-transform group-hover:translate-x-1" aria-hidden>→</span>
              </a>
              <a href="#products" className="inline-flex items-center justify-center gap-3 border border-paper/25 px-8 py-4 tech-label text-paper transition-colors hover:border-paper">
                View Product Range
              </a>
            </motion.div>
          </Reveal>
          ...
          <Reveal delay={0.12}>
            <div className="grid grid-cols-2 gap-px border border-paper/15 bg-paper/15">
              {[
